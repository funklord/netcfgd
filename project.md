...
1. **Give the modem report an addressing source.** The contract is written (`docs/modem-report.md`) and netcfgd reads it — `ncfg status` shows what a helper reported, marked *not applied*, because nothing consumes it yet. What is missing is an `AddressSource` so an `interface` block can say its addresses come from the modem, at which point netcfgd installs them with its own tag and withdraws them when the report empties. **That is a schema change** — a new variant in a frozen enum — so it needs a deliberate `make schema-bless` and a line saying which bump it is.
