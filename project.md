...
- **Beyond a certain load `acl.sh` fails for a reason no test-side wait can fix.** With more busy processes than cores, a fresh plan still shows an `access_control` action pending after the apply that should have made it — the apply did not converge, and netcfgd is not reporting the fake as unanswering, so this is not the one-second ACL deadline being missed. It does not reproduce at ordinary load (0 in 8, and three consecutive full-suite runs on each distribution), and it is recorded here rather than papered over with a longer wait.
