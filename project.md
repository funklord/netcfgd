...
2. **Give the tunnel its credentials.** `--auth-user-pass FILE` takes two lines that netcfgd would write under `/run` at 0600 from a `SecretRef` — the same trade the hostapd passphrase and the `pppd` options file already make. Out of the first slice because a `.ovpn` with inline certificates needs none, and that is the case now working. After it, the harder half [0046](docs/decisions/0046-the-ovpn-file-is-the-operators.md) names: who configures the tunnel's addresses, which PPPoE has open too and which the `/run` report the modem helper uses would answer for both at once.
