...
**The suite runs on Alpine too** ([0100](docs/decisions/0100-kill-0-calls-a-zombie-alive.md)): musl, busybox `ash`, in a container whose pid 1 does not reap. With the packages Alpine has — including hostapd and wpa_supplicant 2.11, both newer than Debian's — 27 scripts pass and three fail for reasons that are not netcfgd's ([0101](docs/decisions/0101-a-fake-on-path-does-not-fake-what-netcfgd-finds-in-sbin.md)): `tunnel.sh`'s own `.ovpn` is a static-key configuration openvpn 2.7 refuses to start on and 2.8 will remove, `ppp.sh` fails one check, and `slaac.sh`'s dnsmasq does not start under `unshare -rn` on either distribution. That combination is the hostile one and is worth keeping: it is where `kill -0` was caught calling a zombie alive. `docker run --privileged -v $PWD:/repo alpine:latest`, plus `iproute2 python3 util-linux`. The skip messages still name Debian packages, which is advice that is wrong there.
