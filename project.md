...
2. **A second look at what `ncfg explain` can now answer.** Four things changed under it this session — reported addresses and routes, a delegation's prefix, what a daemon was started with, and three new "is it still current" booleans — and `explain` learned about the first pair only. An operator asking why an access point restarted, or why a tunnel dropped, gets nothing from the command whose whole job is that question.
