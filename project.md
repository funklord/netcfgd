...
1. **Build the modem helper** ([0044](docs/decisions/0044-the-modem-helper-is-contained-the-way-an-adapter-is.md)): its own workspace and binary, D-Bus to ModemManager, reporting to netcfgd through `/run` the way a DHCPv6 client's hook already does. The core gains nothing — no D-Bus, no glib, no crate. Start with the reporting contract, since that is netcfgd's only half and the one a fixture can exercise.
