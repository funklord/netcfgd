...
1. **Give the modem report its gateway and its DNS.** `config = "modem"` installs the reported *addresses* and withdraws them when the bearer drops. The report's `gateway=` and `dns=` lines are read and shown and not yet applied, so a modem-only host comes up with an address and no route. The DNS half is the first real exercise of decision 0006 rule 4 — "DNS merges in list order" has only ever been implemented for what the document says, and a source contributing nameservers means synthesising a `DnsPolicy` from an observation and deciding what happens when the interface also sets one.
