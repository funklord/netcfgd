...
-- whatever wrote /etc/resolv.conf still owns it". **Setting it from the GUI is
not done**: reading needed no protocol change and writing needs a request, a
privilege tier and a config-write path, which is a design decision rather than
a patch.
