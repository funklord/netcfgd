...
- **A watcher that never sees "up" passes every test for seeing "down".** The monitor mode's drop detection was checked by dropping the bearer and asserting it noticed — and one character wrong in the label it matches makes it decide the bearer is down on the *first* poll, which satisfies all of that. The missing assertion was the positive one: that it is still running while the bearer is up. The general shape is the one this project keeps meeting, in its least obvious form yet — a negative check needs its positive, even when the negative is the interesting half.
...
2. **Decide who configures a tunnel's addresses**, the harder half [0046](docs/decisions/0046-the-ovpn-file-is-the-operators.md) names. OpenVPN and `pppd` both configure their own interface today and netcfgd observes — so netcfgd is the single writer everywhere except the two places a daemon got there first. `--route-noexec --ifconfig-noexec` plus an `--up` script reporting through `/run` would close it, and the report format already exists: `docs/modem-report.md` was written for a modem helper and says nothing modem-specific. Answering it for both tunnel types at once is worth more than answering it for either.
