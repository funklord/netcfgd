...
1. **Write a modem helper**, which is now the only missing piece: `config = "modem"` installs the reported addresses, the default route and the nameservers, and withdraws them when the bearer drops. Nothing writes `/run/netcfgd/modem/<interface>` yet except the test. The cheapest real one is a shell script around `mbimcli --connect` and `--query-ip-configuration`; `umbim` is the one that fits a 16 MB router. Neither needs a bus.
