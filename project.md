...
2. **`pppd`'s `usepeerdns`, which now has an answer waiting for it.** [0049](docs/decisions/0049-a-server-may-name-resolvers-not-where-queries-go.md) settled the hard half for a tunnel — report the servers, leave the routing to the document — and `pppd` asks the identical question one option over. It also needs a check nobody has made: `usepeerdns` writes `/etc/ppp/resolv.conf` rather than `/etc/resolv.conf`, which is what would make it safe to turn on, and that is a claim from reading `ipcp.c` rather than from running one.
