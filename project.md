...
1. **Write down the modem reporting contract, then netcfgd's side of it** ([0044](docs/decisions/0044-the-modem-helper-is-contained-the-way-an-adapter-is.md), [0045](docs/decisions/0045-the-contract-is-the-decision-and-the-helper-is-plural.md)). A helper reports the interface, addresses, gateway and nameservers through `/run`, the way a DHCPv6 client's hook already does; netcfgd's half is a file reader and a fixture that writes one. The contract goes in `docs/` as a contract, so a second helper can be written without reading netcfgd's source — because **the helper is plural**: `umbim` on OpenWrt needs `libubox` and no glib or bus at all, `mbimcli` needs no bus, and a ModemManager helper is for machines that already run one. The core gains nothing in any case.
