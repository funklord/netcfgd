...
2. **The request half of prefix delegation.** `PdRequest` is in the model and no `config` entry sets it, so a router cannot ask its ISP for a prefix at all — the `@pd:` consuming half has nothing to consume. It is deliberately not written yet ([0050](docs/decisions/0050-a-delegated-prefix-is-odhcp6cs-to-report.md)): odhcp6c is the only client that can serve it and Debian does not package it, so the syntax could not be exercised end to end here. Whoever does it should have an odhcp6c to hand — OpenWrt in a container is probably the shortest road — and `dhcp6_client` already refuses the pairing that cannot work.
