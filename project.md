...
2. **The other half of what a delegation implies: router advertisements.** A LAN with an address carved out of a delegated prefix has to tell the hosts on it, and `RaPolicy` is the fourth thing frozen at M4 and unwired — `odhcpd` or `radvd` is the backend, decision 0009 names the handoff, and nothing runs one. `tests/live/delegation.sh` is the rig it would extend: the prefix is already on `lan0` by the end of it.
