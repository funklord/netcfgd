...
**Not yet, though.** The version is pinned at **1.0 until netcfgd ships** ([0038](docs/decisions/0038-versioning-starts-at-the-first-release.md)): a version is a promise to consumers and there are none before a release, so counting minor bumps through a schema still being designed measures effort rather than compatibility. Adding a field bumps minor *from the first release onwards*. What keeps a schema change visible meanwhile is the witnesses under `docs/schema/`, which move on every change and have to be blessed deliberately ([0020](docs/decisions/0020-the-freeze-is-two-witnesses.md)) — the mechanism that was doing the work all along. There are **three**, not the two 0020 named: `Observed` had none until a field was added to it and nothing asked to be blessed, and the socket witness had a comment claiming otherwise.
...
The schema version is pinned at 1.0 until the first release ([0038](docs/decisions/0038-versioning-starts-at-the-first-release.md)). That is not a licence to change it quietly — the three witnesses under `docs/schema/` still move on every change and still need a deliberate `make schema-bless`, which was always the mechanism doing the work. The third one, `observed.json`, is new: `Observed` goes over the socket, into `/run` and out of `ncfg status --json`, and nothing had ever pinned it.
...
2. **A tunnel's nameservers, which means deciding split DNS first.** `openvpn` puts a pushed `dhcp-option DNS` in the same environment the routes arrive in, and the report already has a `dns=` key, so the reporting side is three lines. What is not three lines is the right behaviour: a VPN's servers usually want *split* DNS — the company's names through the tunnel and everything else locally — and that is a routing-domain question [0007](docs/decisions/0007-dns-scopes.md) governs. Taking them as a flat list would deliver the wrong thing convincingly, which is why [0048](docs/decisions/0048-a-tunnels-routes-arrive-through-the-report.md) left them. The same question is `pppd`'s `usepeerdns`, so one answer closes both.
3. **A witness for `Plan`.** `Observed` has one now; a `Status` response carries both, and the socket witness pins the envelope only. It is a hole rather than a decision, and it is the same hole that let a field into `Observed` unnoticed.
