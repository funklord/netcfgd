...
2. **A real PPPoE session, which needs real root.** Everything either daemon is told is now checked against the daemon itself, but `/dev/ppp` is `root`-only and the rp-pppoe plugin opens it at load, so nothing here has ever dialled. `sudo sh tests/live/hwsim.sh` is the precedent: a second privileged script could run pppd over a pty pair against a second pppd, which is a real IPCP negotiation with no DSL line, and it is the only thing that would exercise the report end to end.
