...
2. **The TUI, which still shows only what was applied.** Noted below and now the oldest untouched gap: `ncfg status` marks a reported-and-not-applied address and the Devices pane does not, and none of this session's staleness answers reach it either. The TUI being a subset is deliberate ([0025](docs/decisions/0025-the-audited-crate-is-the-libc-boundary-not-netlink.md)); which subset is a choice nobody has revisited since the panes were written.
