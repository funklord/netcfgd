...
| **M7** | NetworkManager shim | `netcfgd-nm`, tier 1 (`nmcli`, `nm-applet`, `plasma-nm` wifi flows). **In progress:** the device half is done -- bus name, object tree, `ObjectManager` at `/org/freedesktop`, every device with its properties, driven by a real `nmcli` in `tests/live/nm.sh`. Access points, connections, activation and the secret agent bridge remain. Each adapter is its own cargo workspace so its dependencies cannot reach the core, enforced by `make nm-containment` ([0027](docs/decisions/0027-the-shim-is-a-separate-workspace-and-libnm-reads-interfaces.md)). |
