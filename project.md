...
**Measured end to end on both distributions.** On Debian, `make live` **exits 0** — 36 scripts pass and two skip (`hwsim` wants `mac80211_hwsim`, `delegation` wants `odhcp6c`), which is the first time it has ever completed. On Alpine every script that can run there passes too: 32 scripts plus both Rust live binaries (`wg` 5, supplicant 12). **`make live` still cannot complete on Alpine, for one reason** — `dhcp.sh` needs busybox's `udhcpd` applet to serve a lease, and Alpine's busybox has `udhcpc`/`udhcpc6` and no server, with no other package providing one. The script's header says it chose busybox's server so as to need "no package that a build machine would not have if it has busybox at all", and the most busybox-native distribution there is falsifies that. `dhcpcd.sh` skips for the same reason, and `NCFG_LIVE=1` turns both into failures. A dnsmasq fallback would fix it and is not free: dnsmasq drops privileges, which `unshare -rn` forbids, so that script would have to make its own namespace the way `slaac.sh` does.
