...
1. **Find out whether `wwan_hwsim` can be driven**, which is the gate in front of the modem backend rather than the backend itself ([0043](docs/decisions/0043-mbim-is-ours-and-the-quirks-are-a-table.md)). The approach is settled -- MBIM, ours, no dependency -- and the open question is whether it can be tested before it is written. `CONFIG_WWAN_HWSIM` exists and is off in Debian's kernel; what it presents to userspace on a kernel built with it on decides whether an MBIM backend is ordinary work or the first thing here with no live test.
