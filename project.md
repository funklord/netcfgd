...
2. **What else only a real session can find.** `pppoe-session.sh` found an unimplemented hang-up in its first run, and the same shape of test does not exist for everything netcfgd starts. A `wpa_supplicant` association has one (`hwsim.sh`, real root) and an `OpenVPN` tunnel has one that needs none (`tunnel.sh`); a `DHCPv6` client and `odhcpd` have neither, and prefix delegation is the one place a report arrives from a client whose hook netcfgd wrote but whose behaviour nothing here has watched.
