...
- **~~And one daemon in about every other `openvpn.sh` run is not stopped at all.~~ Open**, with the evidence gathered: four daemons started, three `signal SIGTERM` received, one alive, its argv carrying `--auth-user-pass`, its management socket gone -- which only the SIGTERM path unlinks -- and `ncfg apply` saying "nothing to do". It is *not* the start/stop race it looks like: waiting for the socket to be bound before stopping changed nothing. The assertion that found it went red half the time and was taken out rather than shipped red; the script's header carries the findings. This is a netcfgd question rather than a test one, and it is the first thing to pick up on the tunnel path.
