...
- **A filter can remove the very thing you are about to claim is absent.** Auditing what the `.deb` installs, `dpkg -c | grep -v "/$"` was used to see the files -- and `dpkg -c` marks directories with a trailing `/`, so that filter removed every directory. The conclusion drawn was that the package never creates `/etc/netcfgd`, which was then written up as a first-five-minutes defect: the `postinst` tells the operator to write `/etc/netcfgd/netcfgd.conf` into a directory nothing had made. A one-line `debian/netcfgd.dirs` was about to be added. **`./etc/netcfgd/` was in the package all along**, put there by `debian/rules` calling `make install`, which is also why the package and `make install` cannot drift apart -- the rules file drives the Makefile rather than repeating a file list. The probe was structurally incapable of showing the thing whose absence it was cited for, which is the failing-check rule wearing an ordinary shell pipeline. **Before concluding something is missing, check that the command you ran could have shown it.**

- **A `pkill -f` pattern matches any process whose command line merely contains it, including the one running the `pkill`.** Cleaning up two orphaned `sleep 300` processes left by a deliberate crash-test, `pkill -f "sleep 300"` killed the shell that issued it: the wrapper's command line contained the string as *data*, because the heredoc being executed mentioned it. The session's own command died with exit 144. This is the mirror of the `openvpn.sh` entry above, where a pattern matched nothing; here it matched the hand holding it. `ps` gives exact pids in the same breath as the evidence that something is orphaned, and killing by pid cannot match a command line that is only talking about the process.

  A related property, expected rather than investigated next time: **deliberately killing or crash-aborting a test leaks its `TestDir`**, because cleanup runs on `Drop` and a process stopped by a signal or a stack overflow never runs it. Two `/tmp/ncfg-*` directories from falsification runs were mistaken for a live-suite leak before their names were read.
...
## 10.42 situ is not the plan; fuzznet is, and the feasibility study becomes a record

**Settled by the copyright holder 2026-09-05, relayed through `claude-guidelines`.** Three statements, and they are direction rather than findings: **netcfgd is not going to use situ**; **it will use fuzznet**, once fuzznet is mature on a single host; and **the transition may want to happen at the same time as netcfgd moves from Rust to C**, rather than as separate work.

**What that changes is the status of a document, not any code.** `doc/remote-access-feasibility.md` §5.0 and the corrections dated 2026-08-08 under it are reasoning about a tool this project will not adopt. They stay -- an evaluation that was made is worth keeping, and the correction it carries about situ's non-goals is right and was hard-won -- but they are a record of what was evaluated rather than a plan to implement from. In particular, decision 6's replacement instruction that the sealed half "should not be written at all" because situ's phases 7 and 8 supply it no longer follows from a premise this project holds.

**The staleness, measured 2026-09-05 rather than asserted**, because "four weeks old" and "wrong" are different claims and only one is worth acting on. There have been **343 commits in situ since 2026-08-08**, the date the document states it checked against. Its citations still resolve -- situ still has a `## 2. Non-goals` and a `26.7 Phase 7: transforms, tier 1 (extern codecs)` -- so nothing here points at a section that has gone, and the staleness is about scope rather than broken references.

**One claim has been overtaken, and only in half.** The document says chunking is "unaddressed and unplanned, not excluded". situ's decision 0030, *accepted and dated 2026-08-08 -- the same day this document says it checked* -- settles it: a `relation` is a named, pure predicate over exactly two views that "holds no state, allocates nothing, and does not know which messages exist". So **unplanned** is wrong and the substance the document was protecting is right: situ would state a chunking invariant and would never reassemble anything. Its worked example is fuzznet's own frame -- `response.head.msg == request.head.msg` and `response.head.index < request.head.chunks`.

That last point survives not using situ, which is why it is worth carrying past the decision above: **the two documents were written on the same day, about the same question, and reached the same correction independently.** That is corroboration of the kind `evidence.md` calls rare -- two hands rather than one -- and it is the half of §5.0 that keeps its value now that the rest is history.

**The Rust-to-C statement is not new to this tree, and the existing analysis is better than a fresh one.** Constraint 4's note already records that the holder may rewrite netcfgd in C or C++, works out that what the constraint really bounds is *audit surface*, and states the part that does not survive: `forbid(unsafe_code)` is checked by the compiler and a directory convention is checked by whoever is looking, so **state the property when restating it and do not claim the guarantee**. What 2026-09-05 adds is only timing -- that the move and the fuzznet adoption may be one piece of work rather than two.
