...
2. **The one place a restart is still a person's job.** Everything netcfgd starts is now compared to what the document says — except an `openvpn` tunnel, whose configuration netcfgd deliberately never reads ([0046](docs/decisions/0046-the-ovpn-file-is-the-operators.md)). An edited `.ovpn` therefore changes nothing until the tunnel restarts for another reason, and unlike the passphrase there is no answer that keeps 0046 intact: netcfgd would have to read the file to compare it. The honest options are a `mtime` on the path the document names, or nothing at all with the limitation written down — and that is a decision, not an implementation.
