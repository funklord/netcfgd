...
2. **The `Delegated` half nobody has watched change.** A prefix that is *renewed as something different* -- an ISP handing out a new block -- should move the LAN's address and what radvd advertises, and `delegation.sh` stops short of that: it checks the prefix arriving and the prefix going, not the prefix changing. The rig is there, kea can be made to hand out a second pool, and it is the case a real line does at three in the morning.
