...
**The suite runs on Alpine too** ([0100](docs/decisions/0100-kill-0-calls-a-zombie-alive.md)): musl, busybox `ash`, in a container whose pid 1 does not reap — 23 scripts pass, 13 skip on packages the image lacks. That combination is the hostile one and is worth keeping: it is where `kill -0` was caught calling a zombie alive. `docker run --privileged -v $PWD:/repo alpine:latest`, plus `iproute2 python3 util-linux`. The skip messages still name Debian packages, which is advice that is wrong there.
