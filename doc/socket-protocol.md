...
`doc/schema/socket.json`** — a witness generated from the daemon's own types,
...
at `{1, 0}` until netcfgd ships ([0038](decision/0038-versioning-starts-at-the-first-release.md)),
...
- **`doc/schema/socket.json`** pins the bytes. It is generated by an
...
| specified by | this document and `doc/schema/socket.json` | a `situ` schema |
...
([0013](decision/0013-three-things-a-caller-may-be-allowed-to-do.md)). Root
...
- **`secret_put` is `admin`, not `wifi`**, even though `wifi_add` carries a
  ...
  *name*: `wifi_add` writes a secret it also names, for a network it is
  ...
  [0042](decision/0042-only-a-key-nobody-can-revoke-stops-a-plan.md)
...
- **Deleting is writing**, so `config_delete` and `secret_delete` are here for
  ...
  ([0042](decision/0042-only-a-key-nobody-can-revoke-stops-a-plan.md)), and
...
([0128](decision/0128-origin-is-which-socket-you-arrived-on.md)). Origin is
...
[0013](decision/0013-three-things-a-caller-may-be-allowed-to-do.md)'s boundary
...
([0117](decision/0117-adding-a-network-is-a-typed-request-not-a-written-file.md)).
...
([0029](decision/0029-a-profile-is-a-projection-and-secrets-do-not-travel.md),
[0031](decision/0031-the-secret-bridge-runs-one-way.md)). The
...
- **The two client implementations share no code**, only this document and
  ...
  [0116](decision/0116-a-client-that-needs-the-model-is-rust.md) settled and
