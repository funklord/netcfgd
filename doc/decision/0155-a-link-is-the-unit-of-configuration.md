...
- **`guard`** goes to the **link**. An earlier draft sent it to the device on
  ...
  **What a guard is grew into a design of its own while this record was being
  written, and it has moved out**: see
  [0156](0156-a-guard-is-one-thing-with-three-origins.md). Pass 1 needs only
  the placement above; everything else there is pass 2.
