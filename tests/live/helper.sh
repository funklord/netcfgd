...
check "with both families' addresses" \
	"$(sed -n 's/^address=//p' "$work/run/reported/wwan0" | tr '\n' ' ')" \
	"10.64.1.23/30 2001:db8::2/64 "
check "both gateways" \
	"$(sed -n 's/^gateway=//p' "$work/run/reported/wwan0" | tr '\n' ' ')" \
	"10.64.1.24 2001:db8::1 "
check "and every nameserver" \
	"$(sed -n 's/^dns=//p' "$work/run/reported/wwan0" | tr '\n' ' ')" \
...
check "and the mtu, which netcfgd does not use yet" \
	"$(sed -n 's/^mtu=//p' "$work/run/reported/wwan0")" "1428"
...
check "and it names no addresses" \
	"$(grep -c '^address=' "$work/run/reported/wwan0" || true)" "0"
...
check "stopping removes the report entirely" \
	"$([ -f "$work/run/reported/wwan0" ] && echo yes || echo no)" "no"
...
waited=0
while [ ! -s "$work/run/reported/wwan0" ]; do
	waited=$((waited + 1))
	[ "$waited" -gt 50 ] && break
	sleep 0.1
done
...
check "and empties the report when the network drops it" \
	"$(grep -c '^address=' "$work/run/reported/wwan0" 2>/dev/null || true)" "0"
