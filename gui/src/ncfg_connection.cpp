    ...
    QString *error)
{
	...
	*out = ncfg_device_config();
	...
	out->members = from_c(found.members);
	out->stp = found.stp != 0;
	out->vlan_filtering = found.vlan_filtering != 0;
	out->bond_mode = from_c(found.bond_mode);
	out->miimon = found.miimon;
	out->parent = from_c(found.parent);
	out->vlan_id = found.vlan_id;
	out->vlan_protocol = from_c(found.vlan_protocol);
	out->peer = from_c(found.peer);
	out->macvlan_mode = from_c(found.macvlan_mode);
	out->vrf_table = found.vrf_table;
	out->tunnel_mode = from_c(found.tunnel_mode);
	out->local = from_c(found.local);
...
bool ncfg_connection::configs(QList<ncfg_config_row> *out, QString *error)
{
	if (!out) {
		return false;
	}
	...
	char message[NCFG_ERROR_MAX];
	ncfg_configs_t answer;

	if (!ncfg_client_configs(client, &answer, message, sizeof(message))) {
		if (error) {
			*error = QString::fromUtf8(message);
		}
		return false;
	...
	for (size_t i = 0; i < answer.count; i++) {
		ncfg_config_row row;
		row.name = QString::fromUtf8(answer.items[i].name ? answer.items[i].name : "");
		row.file = QString::fromUtf8(answer.items[i].file ? answer.items[i].file : "");
		row.text = QString::fromUtf8(answer.items[i].text ? answer.items[i].text : "");
		row.removable = answer.items[i].removable != 0;
		out->append(row);
	}
	ncfg_configs_free(&answer);
	return true;
}
