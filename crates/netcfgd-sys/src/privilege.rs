...
mod tests {
	...
	// The shed itself is `tests/shed.rs`, and it has to be.
	//
	// **A test that disarms the process cannot live in the process the other
	// tests are in.** Where `shed` reaches `Fully` there is an id to become,
	// so glibc broadcasts the change to every thread and the whole binary is
	// uid 65534 from that moment -- which this module's own comment said in
	// as many words and nothing acted on. Rust runs tests as threads in one
	// process, so whichever were still running lost root mid-flight: a
	// `chmod` on a file made a moment earlier became `EPERM`, and a `kill` of
	// a root-owned child became `EPERM` too, which left `wait` blocking on a
	// `sleep 30` and **thirty seconds on the clock of every run of this
	// crate's tests**. Decision 0262.
	//
	// An integration test is its own binary, so the shed disarms nothing but
	// itself.
