...
/* ------------------------------------------------------------------------ *
 * Which scopes a machine's DNS has, which is a rule and not a loop
 * ------------------------------------------------------------------------ */

/*
 * **In the model because it is a function of a document AND an observation**,
 * which is the whole reason it is a rule rather than a loop -- and why it is
 * here rather than in `dns.h`. The Rust keeps it in `netcfgd_model::dns`, and
 * `netcfgd-plan` depends on `netcfgd-model` and on nothing else; this port had
