/*
 * report.c -- the two responses that are a model plus an envelope.
 *
 ...
 *   checked against anything". These two have one. They take the daemon's own
 ...
 *   `ncfg_observed_write_members` and `ncfg_document_write_members` are
 *   published for this, and the status response and `observed.json` cannot
 *   disagree.
 *
 * WHAT AN ABSENT ANSWER IS
 *   Neither of these invents an empty one. An observation with no links reads
 *   as a machine with nothing on it, and a document with no interfaces as a
 *   machine nobody has configured -- and a client cannot tell either from a
 *   daemon that has not looked yet. Both refuse instead, and a refusal is an
 *   answer: `answer.c` turns it into `{"response":"error","message":...}` with
 *   the sentence in it.
 */
