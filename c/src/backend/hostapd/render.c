...
const char *ncfg_hostapd_effective_band(const char *band, const ncfg_optint_t *channel)
{
	if (band) {
		...
		/* `6` reaches here, and so does anything the compiler let through. The
		 * two are told apart by the caller, because "a band this build cannot
		 * do" must stay a different answer from "not a band". */
		return NULL;
	}
    ...
    ncfg_hostapd_unsupported_t *why, char *err, size_t err_size)
{
	...
	if (access_point->band) {
		...
		declared = ncfg_hostapd_effective_band(access_point->band, NULL);
		if (!declared) {
			if (why) {
	...
	band = declared ? declared : ncfg_hostapd_effective_band(NULL, &access_point->channel);
	if (!ncfg_hostapd_channel_in_band(band, access_point->channel.value)) {
		if (why) {
